...
void GraphicalUI::cb_rayCountCheckBox(Fl_Widget* o, void* v)
{
	pUI=(GraphicalUI*)(o->user_data());
	pUI->m_displayRayCount = (((Fl_Check_Button*)o)->value() == 1);

	if (pUI->m_displayRayCount)
    {
    	// activate RayCount slidebar
	    pUI->m_aaSamplesSlider->show();
	    //pUI->m_raycount = true;
    }
	else
	{
	    pUI->m_aaSamplesSlider->hide();
	    //pUI->m_raycount = false;
	}
}


void GraphicalUI::cb_rayCount(Fl_Widget* o, void* v)
{
	((GraphicalUI*)(o->user_data())) -> m_nRayCount=int( ((Fl_Slider *)o)->value() ) ;
}

void GraphicalUI::cb_threadCheckBox(Fl_Widget* o, void* v)
{
	pUI=(GraphicalUI*)(o->user_data());
	pUI->m_displayThreader = (((Fl_Check_Button*)o)->value() == 1);

	if (pUI->m_displayThreader)
    {
    	// activate RayCount slidebar
	    pUI->m_threadSlider->show();
	    //pUI->m_thread = true;
    }
	else
	{
	    pUI->m_threadSlider->hide();
	   // pUI->m_thread = false;
	}
}

void GraphicalUI::cb_threader(Fl_Widget* o, void* v)
{
	((GraphicalUI*)(o->user_data())) ->  m_nThread=int( ((Fl_Slider *)o)->value() ) ;
}

void GraphicalUI::cb_cubeMapButton(Fl_Widget* o, void* v)
{
		pUI=(GraphicalUI*)(o->user_data());
		pUI->m_gotCubeMap = (((Fl_Check_Button*)o)->value() == 1);
		if (pUI->m_gotCubeMap)
	    {
		    pUI -> m_cubeMapChooser ->activateOK();
		    pUI -> raytracer -> m_haveCubeMap = true;
	    }
}


void GraphicalUI::cb_cubemap(Fl_Widget* o, void* v)
{
...
void renderThr(RayTracer* r, int first, int last, int width)
{
	...
	if (pUI->raytracer->sceneLoaded())
	  {
		...
		//****************your code here******************
		//bool check = pUI->isThreading();
		//if (check)
		//{
			//threadsNum = pUI->getThreads();
			...
			/*if (threadsNum == 0)
				...
				for (int i = 1 ; i < threadsNum ; i++)
				{
						...
						first = ceil((height/threadsNum) * i);

						last = ceil((height/threadsNum) * (i+1));
						...
						threadsVec.push_back(std::thread(renderThr, pUI->raytracer, first, last, width));
				}*/
		//}
		...
		for (int y = 0; y < height ; y++)
	    {
		    ...
		    for (int x = 0; x < width; x++)
		    {
				if (stopTrace) 
					break;
				// check for input and refresh view every so often while tracin}
...
GraphicalUI::GraphicalUI() : refreshInterval(10) {
	...
	m_nThread = std::thread::hardware_concurrency();

	if (m_nThread == 0)
		m_nThread  = 1;


	// set up ray count checkbox
	m_aaCheckButton = new Fl_Check_Button(10, 130, 140, 20, "Anti-Aliasing");
	m_aaCheckButton->user_data((void*)(this));
	m_aaCheckButton->callback(cb_rayCountCheckBox);
	m_aaCheckButton->value(m_displayRayCount); 

	// install Ray Count
	m_aaSamplesSlider = new Fl_Value_Slider(10, 150, 180, 20, "Ray Count");
	...
	m_aaSamplesSlider -> value(m_nRayCount);
	m_aaSamplesSlider -> align(FL_ALIGN_RIGHT);
	m_aaSamplesSlider -> callback(cb_rayCount);

	// set up ray count checkbox
	m_threadCheckBox = new Fl_Check_Button(10, 180, 140, 20, "Threading");
	m_threadCheckBox->user_data((void*)(this));
	m_threadCheckBox->callback(cb_threadCheckBox);
	m_threadCheckBox->value(m_displayThreader); 

	// install threads

	m_threadSlider = new Fl_Value_Slider(10, 210, 180, 20, "Threads");
	...
	m_threadSlider -> value(m_nThread);
	m_threadSlider -> align(FL_ALIGN_RIGHT);
	m_threadSlider -> callback(cb_threader);


	// set up cubemap checkbox
	m_cubeMapCheckButton = new Fl_Check_Button(10, 240, 140, 20, "cube map");
	m_cubeMapCheckButton->user_data((void*)(this));
	m_cubeMapCheckButton->callback(cb_cubeMapButton);
	m_cubeMapCheckButton->value(m_gotCubeMap);

	// set up cubemap slider
	m_cubeMapSlider = new Fl_Value_Slider(10, 270, 180, 20, "cubemap");
	m_cubeMapSlider -> user_data((void*)(this));	// record self to be used by static callback functions
	m_cubeMapSlider -> type(FL_HOR_NICE_SLIDER);
	m_cubeMapSlider -> labelfont(FL_COURIER);
	m_cubeMapSlider -> labelsize(12);
	m_cubeMapSlider -> minimum(1);
	m_cubeMapSlider -> maximum(10);
	m_cubeMapSlider -> step(1);
	m_cubeMapSlider -> value(m_nFilterWidth);
	m_cubeMapSlider -> align(FL_ALIGN_RIGHT);
	m_cubeMapSlider -> callback(cb_cubemap);
